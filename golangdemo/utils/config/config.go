...
func Init() {
	...
	fileName = fmt.Sprintf("config_%s.toml", variable.Global.Get(variable.RunMode))
	if variable.Global.Get(variable.ConfPath) == "." {
		ConfigPath = result.Result(os.Getwd()).Unwrap().(string)
	} else {
		ConfigPath = variable.Global.Get(variable.ConfPath).(string)
	}
	if _, err := toml.DecodeFile(fmt.Sprintf("%s/%s", ConfigPath, fileName), &Toml); err != nil {
		log.Fatal("加载配置文件失败")
	}
}
