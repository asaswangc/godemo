...
type AdminCmdbCheck struct {
	...
	CheckName string    `gorm:"column:check_name;type:varchar(50);comment:验证器名称;NOT NULL" json:"verify_name"`
	CheckType string    `gorm:"column:check_type;type:varchar(50);comment:验证器类型;NOT NULL" json:"verify_type"`
	CheckBody string    `gorm:"column:check_body;type:varchar(100);comment:验证器内容;NOT NULL" json:"verify_body"`
...
func (m *AdminCmdbCheck) TableName() string {
	return "admin_cmdb_check"
}

func (m *AdminCmdbCheck) TableNameZh() string {
	return "c资源模型字段验证器"
}
...
func (m *AdminCmdbCheck) QueryByLike(GormDB *gorm.DB) *gorm.DB {
	// 支持模糊查询的字段
	if m.CheckName != "" {
		GormDB = GormDB.Model(&m).Where("check_name like %?%", m.CheckName)
	}
	if m.CheckBody != "" {
		GormDB = GormDB.Model(&m).Where("check_body like %?%", m.CheckBody)
	}
	return GormDB
}
